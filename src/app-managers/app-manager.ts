    ...
    findById(id: string): Promise<App|null> {
        ...
        return this.server.cacheManager.get(`app:${id}`).then(appFromCache => {
            ...
            return this.driver.findById(id).then(app => {
                this.server.cacheManager.set(`app:${id}`, JSON.stringify(app.initialApp), this.server.options.appManager.cache.ttl);
    ...
    findByKey(key: string): Promise<App|null> {
        ...
        return this.server.cacheManager.get(`app:${key}`).then(appFromCache => {
            ...
            return this.driver.findByKey(key).then(app => {
                this.server.cacheManager.set(`app:${key}`, JSON.stringify(app.initialApp), this.server.options.appManager.cache.ttl);
